...
#loop to read over network condigurations from the yaml file: - one way to do hyperparameter optimization
    ...
    toy_sim  = ROOT.TFile.Open(dictionary["data_inputs"]["sim_file"])
    toy_data = ROOT.TFile.Open(dictionary["data_inputs"]["data_file"])
    ...
    xV=dictionary["data_inputs"]["x_ref"]
    yV=dictionary["data_inputs"]["y_ref"]
    aV=dictionary["data_inputs"]["a_ref"]
    bV=dictionary["data_inputs"]["b_ref"]
    
    # simulation distributions
    Nx=dictionary["data_inputs"]["N_x"]
    Ny=dictionary["data_inputs"]["N_y"]
    for ix in range(Nx):
        for iy in range(Ny):
    ...
    # data distributions
    Na=dictionary["data_inputs"]["N_a"]
    Nb=dictionary["data_inputs"]["N_b"]
    for ia in range(Na):
        for ib in range(Nb):
    ...
    if args.train: 
        ...
        encoder_input_dim  = dictionary[conf]["encoder_input_dim"]
        ...
        # flow configuration
        flow_n_dim = dictionary[conf]["flow_n_dim"]
